...
class GetStarredMessages @Inject constructor(
    private val repository: MessageRepository
) {
    operator fun invoke(itemsOrder: ItemsOrder = ItemsOrder.Date(OrderType.Descending)): Flow<List<MessageEntity>> {
        return repository.getStarredMessages().map { messages ->
